...
if __name__ == '__main__':
	if parse_inputs():
		input_path, model_path, output_path, threshold, duration = parse_inputs()
		min_length = seconds_to_frames(duration)
		...
		# output laughs timestamps as CSV
		with open(input_path.replace("noVocals/", '').replace('-music.wav', '') + '_laughs.csv', 'w') as csvFile:
			fieldnames = ['filename', 'laughs', 'start', 'end', 'threshold', 'duration']
			writer = csv.DictWriter(csvFile, fieldnames=fieldnames)
			writer.writeheader()

			for laugh in laughs:
				laugh['threshold'] = threshold
				laugh['duration'] = duration
				print(laugh)
				writer.writerow(laugh)
