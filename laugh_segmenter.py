...
def get_laughter_instances(probs, threshold, min_length):
	instances = []
	current_list = []
...
def format_outputs(input_path, instances, wav_paths):
	outs = []
	for i in range(len(instances)):
		outs.append({'filename': input_path, 'laughs': wav_paths[i], 'start': instances[i][0], 'end': instances[i][1]})
	return outs


def segment_laughs(input_path, model_path, output_path, threshold, min_length):
	print();
	...
	filename = input_path.rsplit('/', 1)[-1]
	print(filename)
	...
	if len(instances) > 0:
		...
		for index, instance in enumerate(instances):
			...
			wav_path = output_path + "/laughs/laugh_" + str(index) + ".wav"
			print(wav_path)
			...
			wav_paths.append(wav_path.replace(output_path, ''))

		return (format_outputs(filename, instances, wav_paths))
